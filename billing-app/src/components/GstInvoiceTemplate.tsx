...
export const GstInvoiceTemplate: React.FC<GstInvoiceTemplateProps> = ({
    ...
    return (
        <div className="rt-root">
            {/* Title */}
            <div className="rt-title">TAX INVOICE</div>
            ...
            <table className="rt-table">
                ...
                <tbody>
                    ...
                    <tr className="rt-summary">
                        ...
                        <td className="rt-right">{r(services.reduce((s, i) => s + Number(i.amount || 0), 0))}</td>
                    </tr>
                    <tr className="rt-summary">
                        <td colSpan={4} className="rt-label">DISCOUNT</td>
                        <td className="rt-right">{r(services.reduce((s, i) => s + Number(i.discount || 0), 0)) > 0 ? `-${r(services.reduce((s, i) => s + Number(i.discount || 0), 0))}` : '0'}</td>
                    </tr>
                    <tr className="rt-summary rt-section-total">
                        <td colSpan={4} className="rt-label"><strong>TOTAL (A)</strong></td>
                        <td className="rt-right"><strong>{r(services.reduce((s, i) => s + Number(i.amount || 0) - Number(i.discount || 0), 0))}</strong></td>
                    </tr>
                </tbody>
            </table>
            ...
            <table className="rt-table rt-products-table">
                ...
                <tbody>
                    ...
                    <tr className="rt-summary">
                        <td colSpan={4} className="rt-label">SUB TOTAL</td>
                        <td className="rt-right">{r(products.reduce((s, i) => s + (Number(i.price || 0) * Number(i.quantity || 1)), 0))}</td>
                    </tr>
                    <tr className="rt-summary">
                        <td colSpan={4} className="rt-label">DISCOUNT</td>
                        <td className="rt-right">{r(products.reduce((s, i) => s + Number(i.discount || 0), 0)) > 0 ? `-${r(products.reduce((s, i) => s + Number(i.discount || 0), 0))}` : '0'}</td>
                    </tr>
                    <tr className="rt-summary rt-section-total">
                        <td colSpan={4} className="rt-label"><strong>TOTAL (B)</strong></td>
                        <td className="rt-right"><strong>{r(products.reduce((s, i) => s + (Number(i.price || 0) * Number(i.quantity || 1)) - Number(i.discount || 0), 0))}</strong></td>
                    </tr>
